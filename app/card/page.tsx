...
import { aggregateTopics, getSchedule, resolveSessions, selectableSessions } from "@/lib/api";
...
async function load(searchParams: SP) {
  ...
  const allIds = selectableSessions(schedule).map((s) => s.id);
  const { name, ids, theme, x, linkedin } = decodeAgenda(sp, allIds);
  const sessions = resolveSessions(schedule, ids);
  return { name, ids: sessions.map((s) => s.id), sessions, theme, x, linkedin, allIds };
}

export async function generateMetadata({ searchParams }: { searchParams: SP }): Promise<Metadata> {
  const { name, ids, sessions, allIds } = await load(searchParams);
  ...
  const ogUrl = `/api/og?${encodeAgenda({ name, ids }, allIds)}`;
  return {
    title,
    description,
    openGraph: {
...
export default async function CardPage({ searchParams }: { searchParams: SP }) {
  const { name, ids, sessions, theme, x, linkedin, allIds } = await load(searchParams);
  ...
  return (
    <CardView
      name={name}
      sessions={sessions}
      editHref={plannerPath({ name, ids, theme, ...(x && { x }), ...(linkedin && { linkedin }) }, allIds)}
      siteHost={siteHost()}
      initialTheme={theme}
      xHandle={x}
      linkedin={linkedin}
